...
# ctc_dist_data = [[534390996, 2070876, 6764093, 301003, 131899, 230493, 372, 131683, 185, 14, 5, 7, 1, 1, 1, 1, 1, 1, 98920, 33], 
# [415440923, 78467737, 22419434, 9060403, 5703998, 3219736, 1220568, 728127, 4051654, 2267962, 473045, 615102, 220346, 85781, 83605, 30645, 31423, 77, 14, 5],
#  [6375664, 24943945, 40634860, 57480966, 82319064, 88627002, 80337444, 60968270, 39833635, 23932038, 14699875, 9382119, 6196443, 3939129, 2278283, 1194167, 611744, 240879, 91353, 33705], 
#  [2942981, 1482805, 1366794, 1313180, 17332507, 7920130, 41349558, 57546936, 69045202, 73911667, 90559945, 68552591, 45762932, 27631628, 21864827, 7674252, 5708995, 1791902, 346390, 15382],
#   [4649809, 530, 530, 530, 18808421, 530, 40622019, 63912796, 80764563, 1506610, 159769984, 59767986, 42279966, 27833421, 28433361, 6815203, 6693137, 1963564, 298321, 9355]]
...
for i, ax in enumerate(axs):
    ...
    x_labels = [ "%.4f"%(min_v + j*step)  if (j)%5==0 or j==0 else "" for j in range(21)]
    y = ctc_dist_data[i] + [0]
    y_step = int(max(y)/10)
    ...
    ax.text(x=x[-8], y=max(y)*0.55, s="$\sigma$: %.3f"%sigmas[i])
    ax.set_xticklabels(x_labels, rotation=30, fontsize=2)
