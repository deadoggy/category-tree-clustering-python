...
with open(config['original_data_path'] + '/review.json') as review_in:
    ...
    while line != '':
        ...
        try:
            ...
            if not user_ids.has_key(user_id):
                user_ids[user_id] = []
            user_ids[user_id].append(bus_id)
        except Exception, e:
            print e.message
        ...
        if count%10000==0:
            print count
        line = review_in.readline()
print "users=%d"%len(set(user_ids))

for uid in user_ids.keys():
    bus_size += len(set(user_ids[uid]))

print float(bus_size) / len(set(user_ids))